...
function handleDSHEvent(type, data) {
  ...
  if (newState && newState !== state.animState) {
    ...
    // Once-states auto-return; for loop states, we need a timeout
    if (!ONCE_STATES[newState]) {
      // Loop states stay active until next event or timeout
      // They'll be replaced by the next event
    }
  }
}

// Listen for DSH session events via SSE or polling
// DSH injects events via window.__dshEvents or custom events
try {
  // Method 1: Listen for custom DSH events
  window.addEventListener('dsh:session-event', function (e) {
    try {
      var d = e.detail || {}
      handleDSHEvent(d.type, d.data)
    } catch (err) {}
  })
  // Method 2: Poll for DSH event bridge (if available)
  if (window.__dshEventBridge) {
    window.__dshEventBridge.on(function (type, data) {
      handleDSHEvent(type, data)
    })
  }
} catch (err) {}
