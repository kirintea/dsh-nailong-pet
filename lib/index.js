...
function apply(ctx) {
  ...
  disposers.push(ctx.on('session/event', (session, event) => {
    try {
      // DEBUG: log all events to see what DSH actually sends
      console.log('[nailong-pet] event:', JSON.stringify({ type: event && event.type, keys: event && event.data ? Object.keys(event.data).join(',') : '' }))
